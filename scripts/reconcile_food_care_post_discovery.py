...
This utility reads existing runtime artifacts only. It never fetches sources,
changes review decisions, or performs publication. By default it prints a
report; ``--write`` persists the operator reconciliation log under the
private data tree.
"""
...
def reconcile_food(root: Path, run_ids: set[str], date_from: str, date_to: str) -> list[dict[str, Any]]:
    ...
    for path in intake_root.glob("*/**/*.json"):
        ...
        for row in _rows(payload, "candidate_rows"):
            disposition = str(row.get("candidate_disposition") or "")
            rows.append({
                "dispatch": "food-line",
                "run_id": run_id,
                "url": str(row.get("source_url") or row.get("canonical_source_url") or ""),
                "title": str(row.get("title") or ""),
                "detection_date": path.parent.name,
                "expected_outcome": "reviewable source-backed finding",
                "actual_disposition": disposition,
                "failure_stage": "food_current_intake",
                "reason_code": str(row.get("candidate_disposition_reason") or row.get("exclusion_reason") or ""),
                "corrected_disposition": disposition if disposition in TERMINAL else "",
...
def reconcile_care(root: Path, run_ids: set[str], date_from: str, date_to: str) -> list[dict[str, Any]]:
    ...
    for date_dir in run_root.iterdir():
        ...
        for run_dir in date_dir.iterdir():
            ...
            for pattern, disposition in (
                ("*.qualified-candidates.json", "retained_for_review"),
                ("*.exclusions.json", "rejected_with_reason"),
                ("*.failed-extractions.json", "deferred_with_reason"),
                ("*.prefilter.json", "deferred_with_reason"),
            ):
            ...
            for raw_id in sorted(raw_ids):
                ...
                rows.append({
                    ...
                    "title": str(row.get("title") or ""),
                    "detection_date": date_dir.name,
                    "expected_outcome": "reviewable source-backed finding",
                    "actual_disposition": disposition,
                    "failure_stage": "care_collection_handoff" if disposition else "care_post_discovery_reconciliation",
                    "reason_code": str(row.get("exclusion_reason") or row.get("normalized_reason") or ""),
                    "corrected_disposition": disposition if disposition in TERMINAL else "",
                    "duplicate_linkage": row.get("duplicate_linkage") or {},
                    "resolution_status": "accounted" if disposition in TERMINAL else "unaccounted",
                })
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dispatch in {"food-line", "both"}:
        rows.extend(reconcile_food(args.root, run_ids, args.date_from, args.date_to))
    if args.dispatch in {"care-line", "both"}:
        rows.extend(reconcile_care(args.root, run_ids, args.date_from, args.date_to))
    payload = {
        "schema_version": "bluefern.post_discovery_reconciliation.v1",
        ...
        "rows": rows,
        "unaccounted_count": sum(row["resolution_status"] == "unaccounted" for row in rows),
    }
    if args.write:
