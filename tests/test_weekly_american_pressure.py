...
def test_validate_pages_view_for_date_passes_with_latest_and_prior(tmp_path, monkeypatch):
    ...
    (ap / "map").mkdir(parents=True, exist_ok=True)
    (ap / "map" / "index.html").write_text("American Pressure Map May 10-May 16, 2026 latest=2026-05-16", encoding="utf-8")
    (ap / "map" / "map_data.json").write_text(json.dumps({"edition_date": "2026-05-16", "pins": [], "needs_location": []}), encoding="utf-8")
