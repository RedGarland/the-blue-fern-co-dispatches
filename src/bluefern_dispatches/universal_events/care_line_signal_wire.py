...
def _event_from_record(
    ...
    *,
    ...
    evidence_text = _text(reviewed_row, "supporting_passage")
    ...
    return SignalWireEvent(
        ...
        publisher=_text(reviewed_row, "source_publisher", "publisher"),
        source_title=_text(reviewed_row, "source_title", "title"),
        ...
        title=_text(proposed_payload, "title"),
        service_line=_text(reviewed_row, "service_line"),
        service_line_normalized=_text(reviewed_row, "service_line_normalized"),
        facility_name=_text(reviewed_row, "facility_name"),
        city=_text(reviewed_row, "city"),
        state=_text(reviewed_row, "state"),
...
def _render_event_page(event: SignalWireEvent) -> str:
    ...
    <p class="lede">{html.escape(event.facility_name)} in {html.escape(event.city)}, {html.escape(event.state)} — {html.escape(event.event_type.replace("_", " "))}.</p>
    <p><a href="/signals/">Back to the signal wire</a></p>
    <section class="section">
      <h2>Event facts</h2>
      <ul>
        <li><strong>Event ID:</strong> {html.escape(event.event_id)}</li>
        <li><strong>Source candidate:</strong> {html.escape(event.candidate_id)}</li>
        <li><strong>Producer record:</strong> {html.escape(event.producer_record_id)}</li>
        ...
        <li><strong>System discovered:</strong> <time datetime="{html.escape(event.system_discovered_at)}">{html.escape(event.system_discovered_at)}</time></li>
        <li><strong>Verified:</strong> <time datetime="{html.escape(event.verification_at)}">{html.escape(event.verification_at)}</time></li>
        ...
        <li><strong>Service line:</strong> {html.escape(event.service_line)}</li>
        <li><strong>Event type:</strong> {html.escape(event.event_type)}</li>
        <li><strong>Verification status:</strong> {html.escape(event.verification_status)}</li>
        <li><strong>Evidence provenance:</strong> {html.escape(event.evidence_provenance_type)} via {html.escape(event.evidence_source_field)} / {html.escape(event.evidence_source_artifact)}</li>
        <li><strong>Evidence decision:</strong> {html.escape(event.review_decision_id)} — {html.escape(event.review_reason)}</li>
        <li><strong>Supersedes decision:</strong> {html.escape(event.supersedes_evidence_decision_id or "none")}</li>
      </ul>
    </section>
    <section class="section">
    ...
    <section class="section">
      <h2>Deterministic provenance</h2>
      <p>This page is deterministically derived from the reviewed record, the Phase 14E promotion preview, and the preserved evidence-review lineage. Repeated runs reuse the same event ID and payload.</p>
      <ul>
        <li><strong>Record fingerprint:</strong> {html.escape(event.record_fingerprint)}</li>
        <li><strong>Review packet fingerprint:</strong> {html.escape(event.review_packet_fingerprint)}</li>
        <li><strong>Canonical source URL:</strong> <a href="{html.escape(event.source_url)}" rel="noopener noreferrer" target="_blank">{html.escape(event.source_url)}</a></li>
      </ul>
    </section>
  </main>"""
...
def _render_index(events: list[SignalWireEvent]) -> str:
    ...
    for event in events:
        ...
        <a href="/events/{html.escape(event.event_id)}/">
          ...
          <span>{html.escape(event.facility_name)} — {html.escape(event.city)}, {html.escape(event.state)}</span>
          <small>{html.escape(event.announcement_date)} | {html.escape(event.service_line)} | {html.escape(event.event_type.replace("_", " "))}</small>
        </a>
      </li>"""
        )
...
def _render_feed(events: list[SignalWireEvent], *, title: str, link: str, description: str) -> str:
    ...
    for event in events:
        ...
        feed_description = (
            f"{event.facility_name} in {event.city}, {event.state} — {event.event_type.replace('_', ' ')} "
            f"announced {event.announcement_date}; effective {event.effective_date}."
        )
        items.append(
            f"""  <item>
        ...
        "drafts": [
            {
                ...
                "event_url": f"{BASE_URL}/events/{event.event_id}/",
                "text": (
                    f"Care Line Signal Wire: {event.title} — {event.facility_name} in {event.city}, {event.state}. "
