...
RECOVERY_REMAINS_RE = re.compile(r"\b(?P<recovered>\d{1,4})\s+(?:bodies|bodies/remains|remains)\s+recovered\b", re.IGNORECASE)
RECOVERY_STILL_MISSING_RE = re.compile(r"\b(?P<missing>\d{1,4})\s+still\s+unrecovered\b", re.IGNORECASE)
RECOVERY_BURIED_RE = re.compile(r"\b(?P<buried>\d{1,4})\s+people\s+believed\s+buried\b", re.IGNORECASE)
RECOVERY_DEMOGRAPHIC_RE = re.compile(r"\b(?P<count>\d{1,4})\s+(?P<label>children|women)\b", re.IGNORECASE)
...
def curate_stories(sources: list[dict[str, Any]], edition_date: str, now: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    def _extract_gaza_story_fragments(source: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        add_fragment(
            ...
            story_scope=base_scope,
            category=base_scope if base_scope == "palestinian_development" else str(source.get("category_hint") or ""),
            development_type="primary_report",
            affected_system=str(source.get("category_hint") or ""),
            location=str(source.get("region_scope") or ""),
        ...
        if "water well and desalination facility" in lowered and "emergency water trucking" in lowered:
            add_fragment(
                title="UNICEF says Gaza water facility is non-operational after nearby strike",
                summary=(
                    "UNICEF reported a Deir al-Balah water well and desalination facility was severely damaged by a nearby strike "
                    "and became non-operational, affecting more than 4,000 people and prompting emergency water trucking."
                ),
                story_scope="core_gaza",
                category="humanitarian",
                development_type="infrastructure_service_loss",
                affected_system="water infrastructure",
                location="Deir al-Balah",
                event_date=str(source.get("published_at") or ""),
                attribution=str(source.get("publisher") or ""),
                uncertainty="reported by UNICEF",
                casualty_counts={},
            )

        if RECOVERY_REMAINS_RE.search(lowered) and RECOVERY_STILL_MISSING_RE.search(lowered):
            recovered = int(RECOVERY_REMAINS_RE.search(lowered).group("recovered"))
            missing = int(RECOVERY_STILL_MISSING_RE.search(lowered).group("missing"))
            buried_match = RECOVERY_BURIED_RE.search(lowered)
            buried = int(buried_match.group("buried")) if buried_match else 0
            demographic_counts = {
                match.group("label").lower(): int(match.group("count"))
                for match in RECOVERY_DEMOGRAPHIC_RE.finditer(lowered)
            }
            add_fragment(
                title="Gaza Civil Defense reports recovered bodies from destroyed homes",
                summary=(
                    f"Gaza Civil Defense reported {recovered} bodies or remains recovered from destroyed homes, "
                    f"with {missing} still unrecovered."
                ),
                story_scope="core_gaza",
                category="humanitarian",
                development_type="recovery_milestone",
                affected_system="civil defense recovery",
                location=str(source.get("region_scope") or "Gaza"),
                event_date=str(source.get("published_at") or ""),
                attribution=str(source.get("publisher") or ""),
                uncertainty="reported by Gaza Civil Defense",
                casualty_counts={
                    "new_deaths": 0,
                    "recovered_remains": recovered,
                    "still_missing": missing,
                    "believed_buried": buried,
                    "children_recovered": demographic_counts.get("children", 0),
                    "women_recovered": demographic_counts.get("women", 0),
                },
            )
        ...
        if not is_wafa and "al-mashahara" in lowered and "tent" in lowered and ("displaced" in lowered or "displacement" in lowered):
            add_fragment(
                ...
                summary=(
                    "A Palestinian was killed and others were injured when shelling targeted a displacement tent in the Al-Mashahara area east of Gaza City."
                ),
                story_scope="core_gaza",
                category="conflict",
                development_type="casualty_event",
                ...
                event_date=str(source.get("published_at") or ""),
                attribution=str(source.get("publisher") or ""),
                uncertainty="reported by WAFA / local sources",
                casualty_counts={"new_deaths": 1, "new_injuries": 1},
            )
    ...
    for index, source in enumerate(sources, start=1):
        ...
        if not bool(relevance.get("passes")):
            ...
            continue
        fragments = _extract_gaza_story_fragments(source)
        for fragment_index, fragment in enumerate(fragments, start=1):
            suffix = f"{index:03d}" if len(fragments) == 1 else f"{index:03d}-{fragment_index:02d}"
            stories.append(
                {
                    "story_id": f"gaza-story-{edition_date}-{suffix}",
                    "title": fragment["title"],
                    "summary": fragment["summary"],
                    "category": fragment["category"],
                    "story_scope": fragment["story_scope"],
                    "score": score,
                    ...
                    "source_records": [
                        {
                            ...
                            **(
                            ...
                            "development_type": fragment.get("development_type"),
                            "affected_system": fragment.get("affected_system"),
                            "attribution": fragment.get("attribution"),
                            "uncertainty": fragment.get("uncertainty"),
                            "casualty_counts": fragment.get("casualty_counts") or {},
                        }
                    ],
                    "generated_at": now,
                    "relevance_terms_matched": relevance.get("matched_terms") or [],
