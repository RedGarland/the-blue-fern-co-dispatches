...
def test_preview_builds_deterministic_editorial_payload(tmp_path: Path) -> None:
    ...
    assert preview_1["card_title"] == "Food Line — August 14, 2026"
    ...
    assert preview_1["embed"] == {
        ...
        "title": "Food Line — August 14, 2026",
    ...
    assert preview_1["post_text"].startswith("Food Line Dispatch — August 14, 2026")
...
def test_preview_written_artifacts_are_stable(tmp_path: Path) -> None:
    ...
    assert payload["embed"]["title"] == "Food Line — August 14, 2026"
...
def test_preview_card_asset_is_1200_by_630() -> None:
...
def test_preview_uses_review_artifact_and_public_summary_without_boilerplate(tmp_path: Path) -> None:
    ...
    assert preview["post_text"].startswith("Food Line Dispatch — August 14, 2026")
