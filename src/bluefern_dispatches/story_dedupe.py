...
def material_update(candidate: dict[str, Any], prior: dict[str, Any] | None) -> tuple[bool, list[str]]:
    ...
    candidate_text = normalize_text(f"{candidate.get('title', '')} {candidate.get('summary', '')}")
    if "board of peace" in candidate_text and (
        "deployment mechanism" in candidate_text
        or "deployment locations" in candidate_text
        or "implementation roadmap could collapse" in candidate_text
        or "advance elements should arrive soon" in candidate_text
    ):
...
def decision_record(
    ...
    return {
        ...
        "watchlist_status": "material_follow_up" if material else "publication_candidate" if include else "duplicate_skip",
