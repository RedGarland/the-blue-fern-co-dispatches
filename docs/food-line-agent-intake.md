...
```powershell
python scripts/run_food_line_discovery_expansion.py --date YYYY-MM-DD --profile daily-current --max-run-minutes 30 --export-agent-inbox --agent-inbox-dir status/food-line/runtime/agent-inbox
```

Validation reports envelope validity, count, invalid URLs, missing evidence or publication dates, within-run duplicates, and fields requiring human review. The private runtime inbox is `status/food-line/runtime/agent-inbox/`; imported inputs are preserved under `status/food-line/runtime/agent-inbox/processed/YYYY-MM-DD/`. No Downloads scan or filesystem watcher is used.
