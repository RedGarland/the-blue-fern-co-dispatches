...
- Queue runtime path: `status/food-line/runtime/current-signal-review.json`
...
`status/food-line/runtime/current-signal-review.json` is the operator working queue and convenience copy. Once a proposal is approved and a dated snapshot exists, the dated snapshot becomes the authoritative historical review provenance for that edition.
