...
def test_food_line_discovery_backfill_summary_reports_window_and_homepage_blockers(tmp_path: Path):
    ...
    assert summary["google_news_resolution_status_counts"]["failed_homepage_or_landing_url"] == 1
...
def test_food_line_discovery_backfill_reports_missing_public_prose_diagnostics(tmp_path: Path):
    ...
    assert sum(summary["source_role_derivation_status_counts"].values()) >= 1
    assert sum(summary["source_role_counts"].values()) >= 1
