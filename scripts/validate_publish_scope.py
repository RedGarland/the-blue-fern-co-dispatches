...
GAZA_RUNNER_LOG_RE = re.compile(r"^logs/runner-gaza-(?P<date>\d{8})-\d{6}\.log$")
...
def _validate_retrospective_release_authority(
    *,
    ...
    v5_approval_path = re.fullmatch(
        r"approvals/food-line/(food-line-[a-z0-9-]+-retrospective-[0-9]{2})-approval-v5\.json",
        approval_path,
    )
    if legacy_approval_path:
        errors.append("obsolete Food Line retrospective V1 approval; renewed V5 approval is required")
    elif v2_approval_path:
        errors.append("obsolete Food Line retrospective V2 approval; renewed V5 approval is required")
    elif v3_approval_path:
        errors.append("obsolete Food Line retrospective V3 approval; renewed V5 approval is required")
    elif v4_approval_path:
        errors.append("obsolete Food Line retrospective V4 approval; renewed V5 approval is required")
    elif not v5_approval_path:
    ...
    if approval_path not in changed or any(
        ...
        errors.append("retrospective approval authority did not originate in a V5-approval-only commit")
    ...
    try:
        ...
        return errors
    required_flags = {
        "schema_version": "food_line_retrospective_approval_v5",
    ...
    if isinstance(approval, dict) and v5_approval_path and approval.get("batch_id") != v5_approval_path.group(1):
...
def _validate_dispatch_archive_scope(
    *,
    paths: Sequence[str],
    dispatch: str,
    context: str,
    ...
    errors: list[str] = []
    if dispatch not in {"food-line", "care-line"}:
        return ["dispatch-archive publish scope is supported only for food-line and care-line"]
    allowed = f"output/site/{dispatch}/archive.html" if context.startswith("source") else f"{dispatch}/archive.html"
    for raw_path in paths:
        path = _normalize_path(raw_path)
        audio, map_artifact, bluesky = _classify_path(path)
        if audio and not allow_audio:
        ...
        if bluesky and not allow_bluesky:
            errors.append(f"{context} path uses Bluesky artifacts without --allow-bluesky: {path}")
        if path != allowed:
            errors.append(f"{context} path is outside the exact {dispatch} archive scope: {path}")
    return errors
...
def _validate_no_update_scope(
    *,
    ...
    pages_repo_root: Path | None = None,
) -> list[str]:
    errors: list[str] = []
    if dispatch != "gaza":
    ...
    for raw_path in paths:
        ...
        if bluesky and not allow_bluesky:
            errors.append(f"{context} path uses Bluesky artifacts without --allow-bluesky: {path}")
        if path in allowed_release_artifacts:
            continue
        if context.startswith("source"):
            runner_log = GAZA_RUNNER_LOG_RE.fullmatch(path)
            if runner_log:
                log_path = source_repo_root / path if source_repo_root is not None else None
                if log_path is not None and not log_path.exists():
                    errors.append(f"{context} Gaza runner log does not exist and cannot be sanctioned runtime evidence: {path}")
                    continue
                log_date = dt.datetime.strptime(runner_log.group("date"), "%Y%m%d").date()
                if log_date == declared_date:
                    continue
                errors.append(f"{context} path is a Gaza runner log for {log_date.isoformat()} outside declared {date_text}: {path}")
                continue
            if path == "output/site/gaza/rss.xml":
                if source_repo_root is None or pages_repo_root is None:
                    errors.append(f"{context} path cannot be classified as unchanged generated byproduct without source and Pages roots: {path}")
                    continue
                source_rss = source_repo_root / path
                pages_rss = pages_repo_root / "gaza/rss.xml"
                if not source_rss.exists() or not pages_rss.exists():
                    errors.append(f"{context} path cannot be verified against Pages RSS: {path}")
                    continue
                if source_rss.read_bytes() == pages_rss.read_bytes():
                    continue
                errors.append(f"{context} generated Gaza RSS differs from Pages and is outside the no-update release scope: {path}")
                continue
    ...
    if is_source_based_retrospective:
        ...
        errors.extend(
            _validate_no_update_scope(
                ...
                allow_bluesky=allow_bluesky,
                source_repo_root=Path(source_repo_root),
                pages_repo_root=Path(pages_repo_root) if pages_repo_root is not None else None,
            )
        )
    else:
        errors.extend(
