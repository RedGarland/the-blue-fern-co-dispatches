...
def _validate_bound_ref(root: Path, prep: Mapping[str, Any], ref_field: str, hash_field: str) -> None:
...
def _validate_release_prep(root: Path, prep: Mapping[str, Any], *, expected_item_id: str) -> dict[str, Any]:
    ...
    for ref_field, hash_field in REQUIRED_PREP_REFS:
        _validate_bound_ref(root, prep, ref_field, hash_field)
    required_fields = (
...
def _load_release_prep(root: Path, binding: Mapping[str, Any]) -> tuple[dict[str, Any], str, str, dict[str, Any]]:
    ...
    normalized = _validate_release_prep(root, payload, expected_item_id=item_id)
...
def _release_item(prep: Mapping[str, Any], rel: str, prep_sha: str, normalized: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "recovery_reconciliation_sha256": prep["recovery_reconciliation_sha256"],
        "editorial_review_ref": prep["editorial_review_ref"],
        "editorial_review_sha256": prep["editorial_review_sha256"],
        "release_decision_ref": prep["release_decision_ref"],
        "release_decision_sha256": prep["release_decision_sha256"],
