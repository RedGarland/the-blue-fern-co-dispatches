...
For one-date Food Line reconciliation, Codex should run the repository controller first. If the controller requests bounded historical research, Codex may perform target-date research outside the controller and write a validated `food_line_historical_reconstruction_input_v1` packet under `data/dispatches/food-line/historical-reconstruction-inputs/`. The controller then validates, archives, reconciles, and terminally accounts for that packet in `--apply` mode. This workflow does not authorize publication, Pages work, production `--apply`, production historical reconstruction, or scheduler changes.
