...
def test_cascadia_task_xml_uses_clean_project_runtime_without_push():
    ...
    assert "'scripts\\run_cascadia_dispatch.py'" in text
    ...
    assert "--weekly-public" in text
    assert "--historical-search" in text
...
def test_cascadia_schedule_docs_use_clean_runtime_fields_without_push():
    ...
    assert r"& '.\.venv\Scripts\python.exe' 'scripts\run_cascadia_dispatch.py'" in section
    assert "CASCADIA_ALLOW_CURL_NO_REVOKE='1'" in section
    assert "CASCADIA_FETCH_BACKEND='auto'" in section
    assert "--weekly-public" in section
    assert "--historical-search" in section
