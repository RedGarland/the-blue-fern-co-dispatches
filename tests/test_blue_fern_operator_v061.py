...
def _config(runner_root: Path) -> operator.OperatorConfig:
...
def test_food_degraded_stale_routes_to_refresh_not_rebuild(tmp_path: Path) -> None:
    ...
    assert plan.safety_checks["status_destination_root"].endswith("repo")
...
def test_executable_refresh_plan_can_be_immediately_applied(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        operator,
        "_build_refresh_status_payload",
        lambda dispatch, **kwargs: {
    ...
    assert receipt.changed_paths == [
        "ops/status/care-line/history/2026-09-21.json",
        "ops/status/care-line/latest.json",
    ]
    assert (repo_root / "ops/status/care-line/latest.json").is_file()
    ...
    assert receipt.validation["status_destination_root"] == str(repo_root.resolve())
    assert receipt.validation["underlying_incident_preserved"] is True
    latest = json.loads((repo_root / "ops/status/care-line/latest.json").read_text(encoding="utf-8"))
...
def test_refresh_does_not_mutate_runner_source(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(operator, "build_status", lambda *_args, **_kwargs: status)
    monkeypatch.setattr(operator, "_build_refresh_status_payload", lambda dispatch, **kwargs: {"dispatch": dispatch, "observed_date": "2026-09-21", "last_exported_at": kwargs["exported_at"]})
...
def test_unexpected_status_path_mutation_fails_closed(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(operator, "check_operator", lambda **_kwargs: operator.OperatorResult("2026-09-21T12:00:00Z", [], [incident]))
    monkeypatch.setattr(operator, "build_status", lambda *_args, **_kwargs: status)
    monkeypatch.setattr(operator, "_build_refresh_status_payload", lambda dispatch, **kwargs: {"dispatch": dispatch, "observed_date": "2026-09-20", "last_exported_at": kwargs["exported_at"]})

    def write_extra(destination_root: Path, dispatch: str, date: str, payload: dict) -> None:
        operator._write_json(destination_root / "ops/status" / dispatch / "latest.json", payload)
        operator._write_json(destination_root / "ops/status" / dispatch / "history" / f"{date}.json", payload)
        operator._write_json(destination_root / "ops/status/system/latest.json", {"unexpected": True})

    monkeypatch.setattr(operator, "_write_refresh_status_payload", write_extra)
    ...
    assert "ops/status/system/latest.json" in receipt.changed_paths
    assert "ops/status/system/latest.json" in receipt.warnings
