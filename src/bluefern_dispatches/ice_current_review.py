...
def _release_candidate(root: Path, item: dict[str, Any], decision: dict[str, Any], event: dict[str, Any]) -> Path:
    ...
    payload = {
        ...
        "relationship_to_previous": item.get("relationship_to_previous"),
        "relationship_reasons": item.get("relationship_reasons") or [],
        "review_queue_last_seen": item.get("last_seen"),
