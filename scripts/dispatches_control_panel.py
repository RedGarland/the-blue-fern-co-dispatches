...
class DispatchesControlPanel:
    ...
    def _build_ap_review_tab(self, frame: ttk.Frame) -> None:
        ...
        btn_row_1.pack(fill=tk.X, padx=10, pady=4)
        scout_btn = ttk.Button(btn_row_1, text="Scout Week", command=self.scout_ap_week)
        scout_btn.pack(side=tk.LEFT)
        report_btn = ttk.Button(btn_row_1, text="Generate Review Report", command=self.generate_ap_review_report)
        report_btn.pack(side=tk.LEFT, padx=6)
        load_btn = ttk.Button(btn_row_1, text="Load Candidates", command=self.load_ap_candidates)
        load_btn.pack(side=tk.LEFT, padx=6)
        save_btn = ttk.Button(btn_row_1, text="Save Review Decisions", command=self.save_ap_review_decisions)
        ...
        btn_row_3 = ttk.Frame(frame)
        btn_row_3.pack(fill=tk.X, padx=10, pady=4)
        generate_btn = ttk.Button(btn_row_3, text="Generate HTML", command=self.generate_ap_html)
        generate_btn.pack(side=tk.LEFT, padx=6)
        open_html_btn = ttk.Button(btn_row_3, text="Open Generated HTML", command=self.open_ap_generated_html)
        open_html_btn.pack(side=tk.LEFT, padx=6)
        publish_btn = ttk.Button(btn_row_3, text="Publish to Pages Locally", command=self.publish_ap_pages_locally)
        publish_btn.pack(side=tk.LEFT, padx=6)
        checklist_btn = ttk.Button(btn_row_3, text="Run Publish Checklist", command=self.run_ap_publish_checklist)
        checklist_btn.pack(side=tk.LEFT, padx=6)
        copy_checklist_btn = ttk.Button(btn_row_3, text="Copy Checklist Report", command=self.copy_ap_publish_checklist)
        copy_checklist_btn.pack(side=tk.LEFT, padx=6)
        push_btn = ttk.Button(btn_row_3, text="Push Pages Live", command=self.push_ap_pages_live)
