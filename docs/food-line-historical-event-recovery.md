...
Current renewal uses `food_line_retrospective_approval_request_v3` and writes
only the owner-derived path
`approvals/food-line/<batch-id>-approval-v3.json`. Existing V1 and V2
approvals remain immutable historical authority records at their original
unversioned and `-approval-v2.json` paths; they are never overwritten,
renamed, or treated as current authority. Planning rejects V1 and V2 with an
explicit V3-renewal-required diagnostic. A renewed V3 request must bind the
exact current protected source commit and
the exact clean current Pages commit, while reusing the already protected
decisions and correction overlays. Creating V3 records approval authority
...
2. Create both V3 approvals from private request JSON, replay them, and merge
   the V3-approval-only PR normally. The corresponding V1 and V2 files remain
