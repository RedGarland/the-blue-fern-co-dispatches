...
def test_homepage_refresh_discovers_all_active_products_and_fills_extra_slots(tmp_path):
    ...
    assert {card.slug for card in cards} == {"gaza", "food-line", "care-line", "cascadia", "american-pressure"}
    assert len(cards) == 7
    assert [card.relative_url for card in cards] == [
        ...
        "/american-pressure/editions/2026-06-15/",
        "/cascadia/editions/2026-05-03/",
    ]
...
def test_homepage_refresh_supports_legacy_manifestless_release_when_listed_publicly(tmp_path):
    ...
    assert [(release.slug, release.edition_date) for release in releases] == [("cascadia", "2026-05-03")]
